...
The optional top-level configuration below changes the local KCP stall timeout:
...
The default is 300 milliseconds; valid integer values are 1 through 60000.
Invalid values prevent startup with a configuration error. Restart Mousehop after
editing this setting. It applies to every local KCP session, incoming and outgoing;
...
Longer timeouts also delay recovery from genuinely stalled input; very small values
can expire before normal feedback (sent every 50ms). No UI setting is added.
