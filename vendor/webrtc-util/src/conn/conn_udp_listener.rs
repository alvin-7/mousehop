            ...
            tokio::select! {
                ...
                result = pconn.recv_from(&mut buf) => {
                    match result {
                        ...
                        Err(err) => {
                            // WSAECONNRESET reports ICMP Port Unreachable for an
                            // earlier datagram (for example when a peer restarts).
                            // It does not invalidate this shared UDP socket.
                            #[cfg(windows)]
                            if matches!(&err, Error::Io(e) if e.0.raw_os_error() == Some(10054)) {
                                log::debug!("ignoring UDP ICMP port-unreachable: {}", err);
                                continue;
                            }
