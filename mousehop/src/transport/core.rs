...
impl Session {
    pub(super) fn recovery_due(&self, now: u64) -> bool {
        self.ready()
            && (self
    ...
    pub(super) fn recovery_probe(&mut self) -> Result<()> {
        let kcp = self.kcp.as_mut().ok_or("recovery KCP missing")?;
        kcp.send(&[0; 8]).map_err(|_| "recovery probe send")?;
        kcp.flush().map_err(|_| "recovery probe flush")
    }
    pub(super) fn recovery_probe_confirmed(&self) -> bool {
        self.kcp.as_ref().is_some_and(|kcp| kcp.wait_snd() == 0)
    }
    /// A completed two-sided recovery handshake replaces all KCP state. The
    /// outer session/epoch envelope is mandatory for packets using this path.
    pub(super) fn recovered(
        ...
        let mut session = Self::with_timeout(dialer, id, conv, now, timeouts);
        session.init(now)?;
        session.hello = true;
        session.ready = true;
        Ok(session)
    ...
    pub(super) fn new(dialer: bool, id: u64, conv: u32, now: u64) -> Self {
        // Historical fault-injection tests pin the original explicit budgets;
        // product defaults are tested separately in config.rs.
        Self::with_timeout(
            dialer,
            id,
            conv,
            now,
            super::Timeouts {
                stall: super::StallTimeout::try_from(300).unwrap(),
    ...
    pub(super) fn with_timeout(
        ...
        conv: u32,
        now: u64,
        timeouts: impl Into<super::Timeouts>,
    ) -> Self {
        ...
        Self {
            ...
            gap_since: None,
            stall_ms: timeouts.stall.milliseconds(),
            peer_timeout_ms: timeouts.peer.milliseconds(),
        }
    }
    #[cfg(test)]
    ...
    pub(super) fn peer_activity(&mut self, now: u64) {
        if self.ready {
            self.last_peer = now;
        }
    }
    ...
    pub(super) fn tick(&mut self, now: u64) -> Result<()> {
        ...
        let input_stalled = self
            ...
            || self
                ...
                .is_some_and(|t| now.saturating_sub(t) > self.stall_ms);
        if self.ready
            && (now.saturating_sub(self.last_peer) > self.peer_timeout_ms || input_stalled)
        {
            log::warn!(
                "KCP stall: input_limit={}ms peer_limit={}ms peer_age={}ms ack_progress_age={:?}ms oldest_sent={:?}ms oldest_consuming={:?}ms gap_age={:?}ms pending={} consuming={} sent={} received={} consumed={}",
                self.stall_ms,
            ...
            return self.fail(if input_stalled {
                "reliable input stalled"
            } else {
                "peer activity timed out"
            });
        }
...
mod output_tests {
    use super::*;
    #[test]
    fn r11_r14_exact_600ms_input_boundary_and_idle_does_not_recover() {
        let mut session =
            Session::recovered(true, 42, 7, 0, super::super::Timeouts::default()).unwrap();
        assert!(!session.recovery_due(600));
        assert!(!session.recovery_due(2999));
        session.send(&[1], 0).unwrap();
        assert!(!session.recovery_due(599));
        assert!(session.recovery_due(600));
    }
