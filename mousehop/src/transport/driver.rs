use super::{InputTransport, StallTimeout, core::Session, motion};
...
impl Receipt {
    ...
    pub(crate) fn complete(self) {
        if !self.valid() || self.completed.try_send(self.sequence).is_err() {
            log::warn!(
    ...
    pub(crate) async fn after(self, done: tokio::sync::oneshot::Receiver<()>) {
        tokio::select! {
            ...
            _ = tokio::time::sleep_until(self.deadline) => self.cancel.cancel(),
            result = done => {
                if result.is_ok() { self.complete(); } else { self.cancel.cancel(); }
            }
        }
    }
}
...
pub(super) fn attach_required(conn: ArcConn, dialer: bool) -> ArcConn {
    attach_with_timeout(conn, dialer, StallTimeout::default())
}

fn attach_with_timeout(conn: ArcConn, dialer: bool, timeout: StallTimeout) -> ArcConn {
    ...
    spawn_local(run(
        conn, dialer, commands, deliver, ready, status, cancel, timeout,
    ));
    peer
}
...
async fn run(
    ...
    let (completed, mut completions) = mpsc::channel::<u64>(QUEUE);
    ...
    let writer = spawn_local({
        ...
        async move {
            ...
            loop {
                ...
                let packet = if let Some(packet) = if prefer_motion {
                    motion_slot.take()
                } else {
                    None
                } {
                ...
                let send_timeout = if kcp_selected.load(Ordering::Acquire) {
                    timeout.duration()
                } else {
                    StallTimeout::default().duration()
                };
                let deadline = packet.deadline.min(Instant::now() + send_timeout);
    ...
    let mut session = Session::with_timeout(dialer, id, conv, 0, timeout);
    ...
    let result: std::result::Result<(), &'static str> = async {
        loop {
            ...
            tokio::select! {
                _ = cancel.cancelled() => break,
                _ = tick.tick() => {
                    if reliable == Some(true) { session.tick(start.elapsed().as_millis() as u64)?; }
                    else if reliable.is_none() && start.elapsed() > Duration::from_secs(6) {
                        return Err("transport Hello timeout");
                    }
                },
                Some(sequence) = completions.recv() => session.processed(sequence, now)?,
                permit = wire_tx.reserve(), if direct_output.is_some() || session.has_output() => {
                    ...
                    if let Some(packet) = direct_output.take() { permit.send(packet); }
                        ...
                        permit.send(WirePacket { bytes, deadline: start + Duration::from_millis(created) + timeout.duration() });
                    }
                }
                command = commands.recv(), if direct_output.is_none() && (reliable != Some(true) || !session.ready() || session.can_send()) => {
                    let Some(mut bytes) = command else { break; };
                    ...
                    if direct(&event) {
                        if let ProtoEvent::Hello { magic, commit, capabilities } = event {
                            let (b,n): ([u8;MAX_EVENT_SIZE],usize) = ProtoEvent::Hello {
                                magic, commit, capabilities: capabilities | CAP_KCP_INPUT_V1 | CAP_UDP_MOTION_V1
                                    | if dialer { CAP_KCP_REQUEST } else { 0 } }.into();
                            bytes = b[..n].to_vec();
                        }
                        let io_timeout = if kcp_selected.load(Ordering::Acquire) { timeout.duration() } else { StallTimeout::default().duration() };
                        direct_output = Some(WirePacket { bytes, deadline: Instant::now() + io_timeout });
                    } else if reliable == Some(false) {
                        direct_output = Some(WirePacket { bytes, deadline: Instant::now() + StallTimeout::default().duration() });
                    } else if hybrid == Some(true) {
                        let (unreliable, bytes) = motion_sender.pack(event, bytes)?;
                        if unreliable {
                            motion_slot.replace(WirePacket { bytes, deadline: Instant::now() + timeout.duration() });
                        } else { session.send(&bytes, start.elapsed().as_millis() as u64)?; }
                    } else { session.send(&bytes, start.elapsed().as_millis() as u64)?; }
                }
                permit = deliver.reserve(), if !pending.is_empty() || session.has_received() || (session.ready() && motion_receiver.ready()) => {
                    ...
                    if pending.is_empty() {
                        if let Some((sequence, mut bytes)) = session.receive() {
                            ...
                            pending.push_back(Delivery { bytes, receipt: Some(Receipt { sequence,
                                completed: completed.clone(), cancel: cancel.clone(), deadline: Instant::now() + timeout.duration() }) });
                        } else if let Some(bytes) = motion_receiver.take()? {
                            pending.push_back(Delivery { bytes, receipt: None });
                        }
                    }
                    if let Some(delivery) = pending.pop_front() { permit.send(delivery); }
                }
                packet = input_rx.recv(), if session.can_input() => {
            ...
            let is_ready = reliable == Some(false) || session.ready();
            ready.store(is_ready, Ordering::Release);
            if is_ready {
                *status.lock().unwrap() = if reliable == Some(false) { "Legacy" } else { "KCP" }.to_owned();
            }
            if direct_output.as_ref().is_some_and(|p| Instant::now() >= p.deadline) {
                return Err("DTLS output stalled");
            }
        }
        Ok(())
    }.await;
...
impl Conn for Peer {
    ...
    async fn send(&self, buf: &[u8]) -> Result<usize> {
        ...
        let queued = tokio::select! {
            ...
            result = tokio::time::timeout(self.timeout, self.send.send(buf.to_vec())) => {
...
mod tests {
    ...
    impl Conn for MemoryConn {
        ...
        async fn send(&self, b: &[u8]) -> Result<usize> {
            if b.first() == Some(&TAG) && self.drop_transport.load(Ordering::Relaxed) {
                return Ok(b.len());
            }
        ...
        tokio::task::LocalSet::new()
            .run_until(async {
                tokio::time::timeout(Duration::from_secs(5), async {
                    ...
                    let a = attach_required(Arc::new(client.unwrap()), true);
                    let b = attach_required(accepted.unwrap().0, false);
                    assert!(
                        !raw(&b)
                            .as_any()
